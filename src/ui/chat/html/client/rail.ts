...
(function (NS) {
  ...
  function measure() {
    ...
    for (var i = 0; i < marks.length; i++) {
      ...
      dot.hidden = false;
      var y = node.offsetTop - messagesEl.offsetTop;
      // The dot element carries the -half-size margin, so top is its center.
    ...
    lineEl.style.top = (points[0].y + 6) + 'px';
    lineEl.style.height = Math.max(0, points[points.length - 1].y - points[0].y) + 'px';
  }
